...
func ExtractLogs(ctx context.Context, client *scalingo.ScalingoClient, appName, targetTimestamp string, lineCount int, hoursCount int) (string, error) {
	...
	extractFunc := func(statusCh chan status.Message, errorCh chan status.ErrorMessage, finishCh chan status.FinishMessage) error {
		...
		// Update status
		status.Update(statusCh, "checking available log archives")
		...
		if !targetTime.IsZero() {
			...
			if err != nil {
				...
				status.Update(statusCh, "warning: failed to fetch archives list, using recent logs only")
				...
				if err != nil {
					status.Update(statusCh, "warning: failed to parse archive end time, using both recent and archived logs")
		...
		status.Update(statusCh, "fetching recent logs")
		liveLogsCount, err = fetchRecentLogs(ctx, client.Client, appName, tempOutputFile)
		if err != nil {
			return status.ReportError(ctx, errorCh, err)
		}
		status.Update(statusCh, "fetched recent logs", liveLogsCount)

		// Get archived logs if needed
		if needsArchivedLogs {
			status.Update(statusCh, "fetching archived logs")
			...
			status.Update(statusCh, "fetched archive logs", archiveLogsCount)
		}
		...
		status.Update(statusCh, "sorted total lines", totalLines)
		...
		if targetTimestamp != "" {
			if hoursCount > 0 {
				...
				status.Update(statusCh, "filtered logs", filteredLineCount)
			} else if lineCount > 0 {
